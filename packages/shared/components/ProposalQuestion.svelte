<script lang="typescript">
    ...
    $: showMargin = isOpened || ((votedAnswerValue || votedAnswerValue === ABSTAIN_VOTE_VALUE) && !isOpened) || winnerAnswerIndex !== undefined

    function calculatePercentagesFromAllVotes(): void {
        if ($selectedProposal?.status === ProposalStatus.Upcoming || $selectedProposal?.status === ProposalStatus.Commencing) {
            return
        }
