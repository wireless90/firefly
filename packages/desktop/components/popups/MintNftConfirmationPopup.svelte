<script lang="ts">
    ...
    onMount(async () => {
        try {
            await _onMount()
            switch ($mintNftDetails.outputType) {
                case OutputType.Alias:
                    await prepareAliasOutput()
                    break
                case OutputType.Nft:
        ...
        <nft-details class="flex flex-col justify-center items-center space-y-4">
            ...
            <activity-details class="w-full h-full space-y-2 flex flex-auto flex-col flex-shrink-0">
                ...
                {#if activeTab === Tab.Transaction}
                    {#if $mintNftDetails.quantity > 1}
                        <KeyValueBox keyText={localize('general.quantity')} valueText={$mintNftDetails.quantity} />
                        <KeyValueBox
