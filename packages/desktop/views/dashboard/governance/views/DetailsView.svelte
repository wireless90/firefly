<script lang="typescript">
    ...
    function setCurrentAndTotalVotes(): void {
        ...
        if (selectedProposalOverview) {
            ...
            totalVotes = proposalState.status === ProposalStatus.Commencing ? (parseInt(lastActiveOverview.amount, 10) ?? 0) : votesSum
        }
    }
    ...
    function getVoteButtonText(): string {
        if ($selectedProposal?.status === ProposalStatus.Upcoming) {
            const millis = milestoneToDate($networkStatus.currentMilestone, $selectedProposal.milestones[ProposalStatus.Commencing]).getTime() - new Date().getTime()
